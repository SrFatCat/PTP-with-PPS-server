...
class NmeaSerialThread(NmeaSrvThread):
    ...
    def pulsePPS(self, pps_data, ser):
        cur_time=time.time()
        if pps_data['time'] == 0:
            ser.setRTS(False)
            pps_data['time']=cur_time
            pps_data['is_start'] = True
            #print(cur_time, pps_data['is_start'])
        else:
            if pps_data['is_start']:
                if cur_time - pps_data['time'] >= 0.1:
                    #print(cur_time - pps_data['time'], pps_data['is_start'])
                    pps_data['is_start'] = False
                    ser.setRTS(True)
            else:
                if cur_time - pps_data['time'] >= 1:
                    #print(cur_time - pps_data['time'], pps_data['is_start'])
                    pps_data['is_start'] = True
                    ser.setRTS(False)
                    pps_data['time'] = cur_time
    ...
    def run(self):
        ...
        try:
            with serial.Serial(self.serial_config['port'], baudrate=self.serial_config['baudrate'],
                ...
                print('RTS as PPS activate')
                ser.setRTS(True)
                pps_data={'is_start':True, 'time':0 }
                pause_t = 1
                if self.serial_config["baudrate"] >= 9600: 
                    pause_t=0.1
                while True:
                    timer_start = time.perf_counter()
                    with self._lock:
                        ...
                        if self.speed and self.speed != self._speed_cache:
                            ...
                            self._speed_cache = self.speed
                        nmea_list = [f'{_}' for _ in next(self.nmea_object)]
                        self.pulsePPS(pps_data, ser)
                        for nmea in nmea_list:
                            ser.write(str.encode(nmea))
                            self.pulsePPS(pps_data, ser)
                    while(time.perf_counter() - timer_start < pause_t) :
                        self.pulsePPS(pps_data, ser)    
