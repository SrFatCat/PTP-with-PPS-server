...
def serial_config_input(args) -> dict:
    ...
    while True:
        if platform_os.lower() == 'linux':
            ...
            print(f'\n### Choose Serial Port [{def_port}]: ###')
            try:
                if def_port:
                    serial_set['port'] = def_port
                else:
                    serial_set['port'] = input('>>> ')
                print(f'Choice {def_port}')
    ...
    while True:
        ...
        try:
            if args.baudrate:
                serial_set['baudrate'] = args.baudrate
            else:    
                serial_set['baudrate'] = input('>>> ')
            print('Choice ', serial_set['baudrate'])
        except KeyboardInterrupt:
            print('\n\n*** Closing the script... ***\n')
            sys.exit()
